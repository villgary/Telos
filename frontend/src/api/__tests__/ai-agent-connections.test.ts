...
describe('cloud connection API client', () => {
  ...
  it('listCloudConnections hits /ai-agents/discovery', async () => {
    getMock.mockResolvedValue({ data: { total: 0, connections: [] } })
    await listCloudConnections()
    expect(getMock).toHaveBeenCalledWith('/ai-agents/discovery')
  })
  ...
  it('createCloudConnection sends api_key only in POST body', async () => {
    ...
    expect(postMock).toHaveBeenCalledWith('/ai-agents/discovery',
  ...
  it('rotateCloudConnection hits /rotate', async () => {
    ...
    expect(postMock).toHaveBeenCalledWith('/ai-agents/discovery/7/rotate',
      { api_key: 'new-key' })
  })
  ...
  it('getCloudConnectionAudit passes limit/offset', async () => {
    ...
    expect(getMock).toHaveBeenCalledWith('/ai-agents/discovery/2/audit',
